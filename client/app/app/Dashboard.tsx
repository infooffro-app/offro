...
const PAGE_LIMIT = 10; // ← Load 10 offers per page
...
export default function DashboardScreen() {
  ...
  const [loadingMore, setLoadingMore] = useState(false); // loading indicator at bottom
  const [hasMoreData, setHasMoreData] = useState(true);  // false when no more pages
  ...
  // Reset to page 1 whenever filter/search/radius/category changes
  useEffect(() => {
  ...
  // ── Reset pagination & fetch fresh ──
  const resetAndFetch = () => {
    setOffers([]);
    setPage(1);
    setHasMoreData(true);
    fetchOffers(1, true); // page=1, reset=true
  };
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // fetchOffers — core pagination function
  // pageNum: which page to fetch
  // isReset: true = replace list, false = append to list
  // ─────────────────────────────────────────────────────────────────────────
  const fetchOffers = async (pageNum = 1, isReset = false) => {
    ...
    try {
      ...
      // Handle both array and { data: [], total: n } response formats
      const newOffers = Array.isArray(data) ? data : (data.data || []);
      const total = data.total || newOffers.length;
      ...
      const loadedCount = isReset ? newOffers.length : (page - 1) * PAGE_LIMIT + newOffers.length;

      if (newOffers.length < PAGE_LIMIT) {
        setHasMoreData(false); // Got fewer than limit = no more pages
      } else {
        setHasMoreData(true);
      }
  ...
  const renderOfferCard = ({ item, index }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          router.push({
            pathname: '/app/OfferDetailsScreen',
            ...
            <View style={styles.cardMeta}>
              {item.coupon_code && (
                <View style={[styles.couponTag, { backgroundColor: couponBg, borderColor: couponBorder }]}>
                  ...
                  <Text style={[styles.couponCode, { color: couponTextColor }]}>{item.coupon_code}</Text>
                </View>
              )}
              <View style={styles.expiryRow}>
                <Text style={styles.expiryIcon}>📅</Text>
                <Text style={styles.expiryText}>Valid till {formatDate(item.valid_until)}</Text>
              </View>
            </View>
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Called when user scrolls to end of FlatList
  // ─────────────────────────────────────────────────────────────────────────
  const handleLoadMore = useCallback(() => {
    ...
    setPage(nextPage);
    fetchOffers(nextPage, false); // false = append, don't reset
  }, [page, loadingMore, hasMoreData, userLat, userLng, searchRadius, searchQuery, selectedCategory]);
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Footer component — shows loading spinner or "no more data" message
  // ─────────────────────────────────────────────────────────────────────────
  const renderFooter = () => {
    if (loadingMore) {
      return (
  ...
  //             <Text style={styles.menuAvatarText}>{userName?.[0]?.toUpperCase() || 'G'}</Text>
  //           </View>
  //           <View>
  ...
  //         {[
  //           { icon: '👤', label: 'My Profile', action: () => router.push('/app/ViewProfileScreen') },
  //           { icon: '🏪', label: 'Add Shop', action: () => router.push('/app/AddShop') },
  //           { icon: '➕', label: 'Post Offers', action: () => router.push('/app/AddOffer') },
  //           { icon: '🏪', label: 'Shop Details', action: () => router.push('/app/MyShopsScreen') },
  //           { icon: '📋', label: 'My Offers', action: () => router.push('/app/MyOffer') },
  //           { icon: '🔐', label: 'Change Password', action: () => router.push('/app/ChangePasswordScreen') },
  //           { icon: '⚙️', label: 'Settings', action: () => { } },
  //         ].map(({ icon, label, action }) => (
  //           <TouchableOpacity key={label} style={styles.menuItem} onPress={() => { action(); setProfileMenuVisible(false); }}>
  ...
  //         <TouchableOpacity style={styles.menuItem} onPress={() => { AsyncStorage.removeItem('userToken'); setProfileMenuVisible(false); router.push('/login'); }}>
...
//               onPress={() => { action(); setProfileMenuVisible(false); }}
//             >
//               <Text style={styles.menuItemIcon}>{icon}</Text>
//               <Text style={styles.menuItemText}>{label}</Text>
//               <Text style={styles.menuItemArrow}>›</Text>
...
const renderProfileMenu = () => (
  ...
  >
    <TouchableOpacity
      ...
      onPress={() => setProfileMenuVisible(false)}
    >
      {/* Stop touch from closing when tapping inside */}
  ...
  // ─────────────────────────────────────────────────────────────────────────
  return (
    //  <SafeAreaView style={{ flex: 1, backgroundColor: AXIS_COLORS.white }}>
    <>
        ...
        <View style={styles.container}>
          ...
          <View style={styles.header}>
            ...
            <View style={styles.headerActions}>
              ...
              >
                <Text style={styles.postBtnIcon}>{hasShop ? '➕' : '🏪'}</Text>
                <Text style={styles.postBtnText}>{hasShop ? 'Post Offer' : 'Add Shop'}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.avatarBtn} onPress={() => setProfileMenuVisible(true)}>
          ...
          <View style={styles.searchRow}>
            ...
            <TouchableOpacity
              style={[
                styles.filterBtn,
                (selectedState || selectedDistrict || selectedCity) && styles.filterBtnActive
              ]}
          ...
          {/* ── FlatList with everything inside ── */}
          <FlatList
            data={offers}
            ...
            // ── Pagination ──
            onEndReached={handleLoadMore}        // Triggered when user reaches bottom
            onEndReachedThreshold={0.3}          // Trigger at 30% from bottom
            ListFooterComponent={renderFooter}   // Loading spinner / "end" message
            ...
            // ── Header (filters inside FlatList) ──
            ListHeaderComponent={() => (
              <View>
                {/* Radius Filter - hide when state/city selected */}
                  ...
                  <View style={styles.filterSection}>
                    ...
                    <View style={styles.radiusPills}>
                      {[1, 2, 5, 10].map((r) => (
                        <TouchableOpacity
                          key={r}
                          style={[styles.radiusPill, searchRadius === r && styles.radiusPillActive]}
                          onPress={() => setSearchRadius(r)}
                        >
                          <Text style={[styles.radiusPillText, searchRadius === r && styles.radiusPillTextActive]}>
                            {r} km
                          </Text>
                        </TouchableOpacity>
                      ))}
                ...
                {categories.length > 0 && (
                  <View style={styles.filterSection}>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 8 }}>
                      ...
                      >
                        <Text style={[styles.catPillText, selectedCategory === null && styles.catPillTextActive]}>
                          All
                        </Text>
                      </TouchableOpacity>
                      {categories.map((cat) => (
                ...
                <View style={styles.offersHeader}>
                  ...
                  {offers.length > 0 && (
                    <View style={styles.offersBadge}>
                      <Text style={styles.offersBadgeText}>{offers.length} loaded</Text>
                    </View>
                  )}
                </View>
                ...
                {!loading && offers.length === 0 && (
                  <View style={styles.centerState}>
                    ...
                    <Text style={styles.emptySub}>Try increasing the radius or clear your filters</Text>
                  </View>
                )}
              </View>
            )}
              ...
              <View style={styles.modalContainer}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                  {states.map(s => (
                    ...
                    >
                      <Text style={selectedState === s.id && styles.activeText}>
                        {s.name}
                      </Text>
                    </TouchableOpacity>
                  ))}
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                  {districts.map(d => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.filterChip,
                        selectedDistrict === d.id && styles.filterChipActive
                      ]}
                      onPress={() => {
                    ...
                    >
                      <Text style={selectedDistrict === d.id && styles.activeText}>
                        {d.name}
                      </Text>
                    </TouchableOpacity>
                  ))}
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                  {cities.map(c => (
                    ...
                    >
                      <Text style={selectedCity === c.id && styles.activeText}>
                        {c.name}
                      </Text>
                    </TouchableOpacity>
                  ))}
...
const styles = StyleSheet.create({
  ...
  //   borderBottomWidth: 1, borderBottomColor: AXIS_COLORS.border,
  // },
  header: {
    ...
    justifyContent: 'space-between', // ← greeting left, actions right
    alignItems: 'center', 
  },
  greeting: { fontSize: 18, fontWeight: '800', color: '#fff' },
  headerSub: { fontSize: 12, color: '#fff', marginTop: 2, fontWeight: '500' },
  headerActions: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  postBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 5,
    backgroundColor: AXIS_COLORS.lightBg, borderRadius: 10,
    paddingHorizontal: 12, paddingVertical: 8,
  },
  postBtnIcon: { fontSize: 14 },
  postBtnText: { fontSize: 11, fontWeight: '700', color: AXIS_COLORS.primary },
  avatarBtn: {
    width: 36, height: 36, borderRadius: 18,
    backgroundColor: '#fff',
    justifyContent: 'center', alignItems: 'center',
  },
  avatarText: { color: AXIS_COLORS.primary, fontSize: 14, fontWeight: '800' },
  ...
  //   borderWidth: 1.5, borderColor: AXIS_COLORS.border, height: 46,
  // },
  searchIcon: { fontSize: 16, marginRight: 8 },
  searchInput: { flex: 1, fontSize: 14, color: AXIS_COLORS.text, fontWeight: '500' },
  clearBtn: { fontSize: 16, color: AXIS_COLORS.muted, paddingLeft: 8 },
  ...
  filterLabel: { fontSize: 11, fontWeight: '700', color: AXIS_COLORS.muted, textTransform: 'uppercase', letterSpacing: 0.6 },
  ...
  radiusPill: {
    ...
    backgroundColor: AXIS_COLORS.white, borderWidth: 1.5, borderColor: AXIS_COLORS.border,
    alignItems: 'center',
  },
  radiusPillActive: { backgroundColor: AXIS_COLORS.primary, borderColor: AXIS_COLORS.primary },
  radiusPillText: { fontSize: 11, fontWeight: '700', color: AXIS_COLORS.muted },
  radiusPillTextActive: { color: '#fff' },
  catPill: {
    paddingHorizontal: 14, paddingVertical: 7,
    backgroundColor: AXIS_COLORS.white, borderRadius: 20, marginRight: 7,
    borderWidth: 1.5, borderColor: AXIS_COLORS.border,
  },
  catPillActive: { backgroundColor: AXIS_COLORS.primary, borderColor: AXIS_COLORS.primary },
  ...
  offersLabel: { fontSize: 11, fontWeight: '700', color: AXIS_COLORS.muted, textTransform: 'uppercase', letterSpacing: 0.6 },
  offersBadge: { backgroundColor: '#E8F1F8', borderRadius: 10, paddingHorizontal: 9, paddingVertical: 3 },
  offersBadgeText: { fontSize: 11, fontWeight: '700', color: AXIS_COLORS.primary },

  // ── Offer Card ──
  offerCard: {
    backgroundColor: AXIS_COLORS.white, borderRadius: 14,
  ...
  offerTitle: { fontSize: 13, fontWeight: '700', color: AXIS_COLORS.text, lineHeight: 19, marginBottom: 9 },
  ...
  discountBlock: { borderRadius: 9, paddingHorizontal: 10, paddingVertical: 6, alignItems: 'center', minWidth: 52 },
  discountValue: { fontSize: 16, fontWeight: '900', color: '#fff', lineHeight: 19 },
  ...
  arrowBtn: {
    ...
    backgroundColor: AXIS_COLORS.lightBg, borderWidth: 1.5,
    justifyContent: 'center', alignItems: 'center',
  },
  ...
  footerEnd: {
    flexDirection: 'row', alignItems: 'center',
    paddingHorizontal: 20, paddingVertical: 20, gap: 10,
  },
  footerLine: { flex: 1, height: 1, backgroundColor: AXIS_COLORS.border },
  footerEndText: { fontSize: 11, color: AXIS_COLORS.muted, fontWeight: '600' },
...
menuContainer: {
  ...
  paddingBottom: 40,            // ← fixed bottom padding, no flex/maxHeight
},
menuHandle: {
  ...
  menuAvatar: {
    ...
    backgroundColor: AXIS_COLORS.primary, justifyContent: 'center', alignItems: 'center',
  },
  menuAvatarText: { color: '#fff', fontSize: 20, fontWeight: '800' },
  menuUserName: { fontSize: 15, fontWeight: '800', color: AXIS_COLORS.text },
  menuUserSub: { fontSize: 12, color: AXIS_COLORS.primary, fontWeight: '600', marginTop: 2 },
  menuItem: {
    flexDirection: 'row', alignItems: 'center',
  ...
  menuItemText: { flex: 1, fontSize: 14, fontWeight: '600', color: AXIS_COLORS.text },
  menuItemArrow: { fontSize: 18, color: AXIS_COLORS.muted },
  menuDivider: { height: 1, backgroundColor: AXIS_COLORS.border, marginVertical: 6 },
